...
namespace Gs2.Unity.UiKit.Gs2Mission.SpriteSwitcher
{
    ...
    public partial class Gs2MissionCounterModelMetadataSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyMetadatas.Contains(this._fetcher.CounterModel?.Metadata ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.CounterModel?.Metadata ?? "").StartsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.CounterModel?.Metadata ?? "").EndsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
