...
namespace Gs2.Unity.UiKit.Gs2Mission
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.MissionTaskModels.Count) {
                    this._children[i].SetMissionTaskModel(
                        MissionTaskModel.New(
                            this._fetcher.Context.MissionGroupModel,
                            this._fetcher.MissionTaskModels[i].Name
                        )
                    );
    ...
    public partial class Gs2MissionMissionTaskModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2MissionMissionTaskModelListFetcher>() ?? GetComponentInParent<Gs2MissionMissionTaskModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2MissionMissionTaskModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2MissionMissionTaskModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2MissionMissionTaskModelListFetcher>() ?? GetComponentInParent<Gs2MissionMissionTaskModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
