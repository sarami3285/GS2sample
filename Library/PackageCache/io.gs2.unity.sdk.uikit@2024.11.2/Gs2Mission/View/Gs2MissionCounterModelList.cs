...
namespace Gs2.Unity.UiKit.Gs2Mission
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.CounterModels.Count) {
                    this._children[i].SetCounterModel(
                        CounterModel.New(
                            this._fetcher.Context.Namespace,
                            this._fetcher.CounterModels[i].Name
                        )
                    );
    ...
    public partial class Gs2MissionCounterModelList
    {
        ...
        {
            if (this.prefab == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2MissionCounterModelContext Prefab.");
                enabled = false;
                return;
            }

            this._fetcher = GetComponent<Gs2MissionCounterModelListFetcher>() ?? GetComponentInParent<Gs2MissionCounterModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2MissionCounterModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2MissionCounterModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2MissionCounterModelListFetcher>() ?? GetComponentInParent<Gs2MissionCounterModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
