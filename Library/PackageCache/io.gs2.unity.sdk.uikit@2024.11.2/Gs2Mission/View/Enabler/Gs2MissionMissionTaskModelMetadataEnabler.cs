...
namespace Gs2.Unity.UiKit.Gs2Mission.Enabler
{
    ...
    public partial class Gs2MissionMissionTaskModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.MissionTaskModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.MissionTaskModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.MissionTaskModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.MissionTaskModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
