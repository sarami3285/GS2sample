...
namespace Gs2.Unity.UiKit.Gs2Mission.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Mission/CounterModel/View/Enabler/Properties/Metadata/Gs2MissionCounterModelMetadataEnabler")]
    public partial class Gs2MissionCounterModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.CounterModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.CounterModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.CounterModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.CounterModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
