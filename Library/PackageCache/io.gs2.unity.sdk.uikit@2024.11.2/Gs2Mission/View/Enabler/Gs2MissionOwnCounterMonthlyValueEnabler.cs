...
namespace Gs2.Unity.UiKit.Gs2Mission
{
    ...
    public partial class Gs2MissionOwnCounterMonthlyValueEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMonthlyValues.Contains(this._fetcher.Counter?.Values?.FirstOrDefault(v => v.ResetType == "notReset")?.Value ?? 0));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMonthlyValues.Contains(this._fetcher.Counter?.Values?.FirstOrDefault(v => v.ResetType == "notReset")?.Value ?? 0));
                    break;
                case Expression.Less:
                    this.target.SetActive(this.enableMonthlyValue > (this._fetcher.Counter?.Values?.FirstOrDefault(v => v.ResetType == "notReset")?.Value ?? 0));
                    break;
