...
namespace Gs2.Unity.UiKit.Gs2Version
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.VersionModels.Count) {
                    this._children[i].SetVersionModel(
                        VersionModel.New(
                            this._fetcher.Context.Namespace,
                            this._fetcher.VersionModels[i].Name
                        )
                    );
    ...
    public partial class Gs2VersionVersionModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2VersionVersionModelListFetcher>() ?? GetComponentInParent<Gs2VersionVersionModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2VersionVersionModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2VersionVersionModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2VersionVersionModelListFetcher>() ?? GetComponentInParent<Gs2VersionVersionModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
