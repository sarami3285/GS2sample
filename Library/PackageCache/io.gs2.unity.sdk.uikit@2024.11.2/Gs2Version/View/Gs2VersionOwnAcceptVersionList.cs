...
namespace Gs2.Unity.UiKit.Gs2Version
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.AcceptVersions.Count) {
                    this._children[i].SetOwnAcceptVersion(
    ...
    public partial class Gs2VersionOwnAcceptVersionList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2VersionOwnAcceptVersionListFetcher>() ?? GetComponentInParent<Gs2VersionOwnAcceptVersionListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2VersionOwnAcceptVersionListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2VersionOwnAcceptVersionContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2VersionOwnAcceptVersionListFetcher>() ?? GetComponentInParent<Gs2VersionOwnAcceptVersionListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
