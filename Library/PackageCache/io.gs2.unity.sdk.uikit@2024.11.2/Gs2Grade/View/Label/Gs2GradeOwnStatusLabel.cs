...
namespace Gs2.Unity.UiKit.Gs2Grade
{
	...
	[AddComponentMenu("GS2 UIKit/Grade/Status/View/Label/Gs2GradeOwnStatusLabel")]
    public partial class Gs2GradeOwnStatusLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{gradeName}", $"{this._fetcher?.Status?.GradeName}"
                ).Replace(
                    "{propertyId}", $"{this._fetcher?.Status?.PropertyId}"
                ).Replace(
                    "{gradeValue}", $"{this._fetcher?.Status?.GradeValue}"
                )
            );
        }
    }
    ...
    public partial class Gs2GradeOwnStatusLabel
    {
