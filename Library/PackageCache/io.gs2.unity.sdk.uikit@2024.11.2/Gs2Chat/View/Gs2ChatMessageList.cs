...
namespace Gs2.Unity.UiKit.Gs2Chat
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.Messages.Count) {
                    this._children[i].SetMessage(
                        Message.New(
                            this._fetcher.Context.Room,
    ...
    public partial class Gs2ChatMessageList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2ChatMessageListFetcher>() ?? GetComponentInParent<Gs2ChatMessageListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2ChatMessageListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2ChatMessageContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2ChatMessageListFetcher>() ?? GetComponentInParent<Gs2ChatMessageListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
