...
namespace Gs2.Unity.UiKit.Gs2Chat.SpriteSwitcher
{
    ...
    public partial class Gs2ChatRoomMetadataSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyMetadatas.Contains(this._fetcher.Room?.Metadata ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.Room?.Metadata ?? "").StartsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.Room?.Metadata ?? "").EndsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
