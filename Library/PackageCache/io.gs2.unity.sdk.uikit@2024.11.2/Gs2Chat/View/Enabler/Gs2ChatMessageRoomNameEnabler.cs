...
namespace Gs2.Unity.UiKit.Gs2Chat.Enabler
{
    ...
    public partial class Gs2ChatMessageRoomNameEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableRoomNames.Contains(this._fetcher.Message?.RoomName ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableRoomNames.Contains(this._fetcher.Message?.RoomName ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.Message?.RoomName ?? "").StartsWith(this.enableRoomName));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.Message?.RoomName ?? "").EndsWith(this.enableRoomName));
                    break;
                default:
