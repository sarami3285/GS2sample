...
namespace Gs2.Unity.UiKit.Gs2Chat
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.Subscribes.Count) {
                    this._children[i].SetOwnSubscribe(
                        OwnSubscribe.New(
    ...
    public partial class Gs2ChatOwnSubscribeList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2ChatOwnSubscribeListFetcher>() ?? GetComponentInParent<Gs2ChatOwnSubscribeListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2ChatOwnSubscribeListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2ChatOwnSubscribeContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2ChatOwnSubscribeListFetcher>() ?? GetComponentInParent<Gs2ChatOwnSubscribeListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
