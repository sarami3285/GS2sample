...
namespace Gs2.Unity.UiKit.Gs2Stamina.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Stamina/StaminaModel/View/Enabler/Properties/Metadata/Gs2StaminaStaminaModelMetadataEnabler")]
    public partial class Gs2StaminaStaminaModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.StaminaModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.StaminaModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.StaminaModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.StaminaModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
