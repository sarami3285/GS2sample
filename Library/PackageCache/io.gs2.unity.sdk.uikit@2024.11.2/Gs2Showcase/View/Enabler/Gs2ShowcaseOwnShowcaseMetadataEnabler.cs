...
namespace Gs2.Unity.UiKit.Gs2Showcase.Enabler
{
    ...
    public partial class Gs2ShowcaseOwnShowcaseMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.Showcase?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.Showcase?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.Showcase?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.Showcase?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
