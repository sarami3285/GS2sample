...
namespace Gs2.Unity.UiKit.Gs2Showcase.SpriteSwitcher
{
    ...
    public partial class Gs2ShowcaseOwnShowcaseNameSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyNames.Contains(this._fetcher.Showcase?.Name ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.Showcase?.Name ?? "").StartsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.Showcase?.Name ?? "").EndsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
