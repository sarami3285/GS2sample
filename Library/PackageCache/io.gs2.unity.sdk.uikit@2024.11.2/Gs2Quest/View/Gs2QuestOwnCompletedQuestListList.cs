...
namespace Gs2.Unity.UiKit.Gs2Quest
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.CompletedQuestList.Count) {
                    this._children[i].SetOwnCompletedQuestList(
    ...
    public partial class Gs2QuestOwnCompletedQuestListList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2QuestOwnCompletedQuestListListFetcher>() ?? GetComponentInParent<Gs2QuestOwnCompletedQuestListListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2QuestOwnCompletedQuestListListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2QuestOwnCompletedQuestListContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2QuestOwnCompletedQuestListListFetcher>() ?? GetComponentInParent<Gs2QuestOwnCompletedQuestListListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
