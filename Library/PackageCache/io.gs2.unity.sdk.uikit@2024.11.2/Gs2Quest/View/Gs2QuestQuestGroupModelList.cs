...
namespace Gs2.Unity.UiKit.Gs2Quest
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.QuestGroupModels.Count) {
                    this._children[i].SetQuestGroupModel(
                        QuestGroupModel.New(
                            this._fetcher.Context.Namespace,
                            this._fetcher.QuestGroupModels[i].Name
                        )
                    );
    ...
    public partial class Gs2QuestQuestGroupModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2QuestQuestGroupModelListFetcher>() ?? GetComponentInParent<Gs2QuestQuestGroupModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2QuestQuestGroupModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2QuestQuestGroupModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2QuestQuestGroupModelListFetcher>() ?? GetComponentInParent<Gs2QuestQuestGroupModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
