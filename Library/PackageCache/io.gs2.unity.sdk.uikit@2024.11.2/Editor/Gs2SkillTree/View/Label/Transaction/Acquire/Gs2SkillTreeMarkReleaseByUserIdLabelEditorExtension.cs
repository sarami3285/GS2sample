...
{
    [CustomEditor(typeof(Gs2SkillTreeMarkReleaseByUserIdLabel))]
    public class Gs2SkillTreeMarkReleaseByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2SkillTreeMarkReleaseByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2SkillTreeMarkReleaseByUserIdFetcher>() ?? original.GetComponentInParent<Gs2SkillTreeMarkReleaseByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2SkillTreeMarkReleaseByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2SkillTreeMarkReleaseByUserIdFetcher>();
                }
            }
