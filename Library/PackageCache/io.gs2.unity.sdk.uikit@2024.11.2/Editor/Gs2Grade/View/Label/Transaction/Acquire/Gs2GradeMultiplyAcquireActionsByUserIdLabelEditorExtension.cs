...
{
    [CustomEditor(typeof(Gs2GradeMultiplyAcquireActionsByUserIdLabel))]
    public class Gs2GradeMultiplyAcquireActionsByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2GradeMultiplyAcquireActionsByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2GradeMultiplyAcquireActionsByUserIdFetcher>() ?? original.GetComponentInParent<Gs2GradeMultiplyAcquireActionsByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2GradeMultiplyAcquireActionsByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2GradeMultiplyAcquireActionsByUserIdFetcher>();
                }
            }
