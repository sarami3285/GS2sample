...
{
    [CustomEditor(typeof(Gs2GradeSubGradeByUserIdLabel))]
    public class Gs2GradeSubGradeByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2GradeSubGradeByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2GradeSubGradeByUserIdFetcher>() ?? original.GetComponentInParent<Gs2GradeSubGradeByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2GradeSubGradeByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2GradeSubGradeByUserIdFetcher>();
                }
            }
            ...
            if (userDataFetcher != null) {
                ...
                if (GUILayout.Button("UserData:GradeValue:Changed")) {
                    original.format += "{userData:gradeValue:changed}";
                    GUI.FocusControl("");
                    EditorUtility.SetDirty(original);
                }
            }
