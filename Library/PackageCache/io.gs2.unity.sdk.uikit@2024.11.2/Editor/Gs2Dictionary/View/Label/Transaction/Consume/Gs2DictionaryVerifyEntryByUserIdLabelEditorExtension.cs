...
{
    [CustomEditor(typeof(Gs2DictionaryVerifyEntryByUserIdLabel))]
    public class Gs2DictionaryVerifyEntryByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2DictionaryVerifyEntryByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2DictionaryVerifyEntryByUserIdFetcher>() ?? original.GetComponentInParent<Gs2DictionaryVerifyEntryByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2DictionaryVerifyEntryByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2DictionaryVerifyEntryByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("EntryModelName")) {
                original.format += "{entryModelName}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
