...
{
    [CustomEditor(typeof(Gs2DictionaryDeleteEntriesByUserIdLabel))]
    public class Gs2DictionaryDeleteEntriesByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2DictionaryDeleteEntriesByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2DictionaryDeleteEntriesByUserIdFetcher>() ?? original.GetComponentInParent<Gs2DictionaryDeleteEntriesByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2DictionaryDeleteEntriesByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2DictionaryDeleteEntriesByUserIdFetcher>();
                }
            }
