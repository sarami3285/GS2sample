...
{
    [CustomEditor(typeof(Gs2IdleReceiveByUserIdLabel))]
    public class Gs2IdleReceiveByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2IdleReceiveByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2IdleReceiveByUserIdFetcher>() ?? original.GetComponentInParent<Gs2IdleReceiveByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2IdleReceiveByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2IdleReceiveByUserIdFetcher>();
                }
            }
