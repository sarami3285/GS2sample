...
{
    [CustomEditor(typeof(Gs2JobQueueOwnJobCurrentRetryCountSpriteSwitcher))]
    public class Gs2JobQueueOwnJobCurrentRetryCountSpriteSwitcherEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2JobQueueOwnJobCurrentRetryCountSpriteSwitcher;
            ...
            if (original.expression == Gs2JobQueueOwnJobCurrentRetryCountSpriteSwitcher.Expression.In || original.expression == Gs2JobQueueOwnJobCurrentRetryCountSpriteSwitcher.Expression.NotIn) {
                EditorGUILayout.PropertyField(serializedObject.FindProperty("applyCurrentRetryCounts"), true);
            } else {
                EditorGUILayout.PropertyField(serializedObject.FindProperty("applyCurrentRetryCount"), true);
            }
