...
{
    [CustomEditor(typeof(Gs2InventoryAcquireItemSetWithGradeByUserIdLabel))]
    public class Gs2InventoryAcquireItemSetWithGradeByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventoryAcquireItemSetWithGradeByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2InventoryAcquireItemSetWithGradeByUserIdFetcher>() ?? original.GetComponentInParent<Gs2InventoryAcquireItemSetWithGradeByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2InventoryAcquireItemSetWithGradeByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2InventoryAcquireItemSetWithGradeByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("GradeModelId")) {
                original.format += "{gradeModelId}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
