...
{
    [CustomEditor(typeof(Gs2InventorySetSimpleItemsByUserIdLabel))]
    public class Gs2InventorySetSimpleItemsByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventorySetSimpleItemsByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2InventorySetSimpleItemsByUserIdFetcher>() ?? original.GetComponentInParent<Gs2InventorySetSimpleItemsByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2InventorySetSimpleItemsByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2InventorySetSimpleItemsByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("Counts")) {
                original.format += "{counts}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
