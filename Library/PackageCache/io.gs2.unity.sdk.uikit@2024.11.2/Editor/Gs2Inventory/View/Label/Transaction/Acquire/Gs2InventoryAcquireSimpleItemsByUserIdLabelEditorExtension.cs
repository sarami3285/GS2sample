...
{
    [CustomEditor(typeof(Gs2InventoryAcquireSimpleItemsByUserIdLabel))]
    public class Gs2InventoryAcquireSimpleItemsByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventoryAcquireSimpleItemsByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2InventoryAcquireSimpleItemsByUserIdFetcher>() ?? original.GetComponentInParent<Gs2InventoryAcquireSimpleItemsByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2InventoryAcquireSimpleItemsByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2InventoryAcquireSimpleItemsByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("AcquireCounts")) {
                original.format += "{acquireCounts}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
