...
{
    [CustomEditor(typeof(Gs2InventoryAcquireItemSetByUserIdLabel))]
    public class Gs2InventoryAcquireItemSetByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventoryAcquireItemSetByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2InventoryAcquireItemSetByUserIdFetcher>() ?? original.GetComponentInParent<Gs2InventoryAcquireItemSetByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2InventoryAcquireItemSetByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2InventoryAcquireItemSetByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("AcquireCount")) {
                original.format += "{acquireCount}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("ExpiresAt")) {
                original.format += "{expiresAt}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("CreateNewItemSet")) {
                original.format += "{createNewItemSet}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
