...
{
    [CustomEditor(typeof(Gs2InventoryOwnSimpleItemQuantitySliderLabel))]
    public class Gs2InventoryOwnSimpleItemQuantitySliderLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventoryOwnSimpleItemQuantitySliderLabel;

            if (original == null) return;

            var slider = original.GetComponent<Gs2InventoryOwnSimpleItemQuantitySliderLabel>() ?? original.GetComponentInParent<Gs2InventoryOwnSimpleItemQuantitySliderLabel>(true);
            if (slider == null) {
                EditorGUILayout.HelpBox("Gs2InventoryOwnSimpleItemQuantitySliderLabel not found.", MessageType.Error);
                if (GUILayout.Button("Add Slider")) {
                    original.gameObject.AddComponent<Gs2InventoryOwnSimpleItemQuantitySliderLabel>();
                }
            }
            ...
            if (fetcher == null) {
                ...
                if (fetcher.gameObject.GetComponentInParent<Gs2InventoryOwnSimpleItemList>(true) != null) {
                    ...
                    if (fetcher.Context != null) {
                        fetcher.Context.SimpleItem = EditorGUILayout.ObjectField("SimpleItem", fetcher.Context.SimpleItem, typeof(OwnSimpleItem), false) as OwnSimpleItem;
                        EditorGUI.indentLevel++;
