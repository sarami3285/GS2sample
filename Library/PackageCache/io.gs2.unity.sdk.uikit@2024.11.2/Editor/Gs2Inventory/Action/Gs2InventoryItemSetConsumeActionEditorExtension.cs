...
{
    [CustomEditor(typeof(Gs2InventoryItemSetConsumeAction))]
    public class Gs2InventoryItemSetConsumeActionEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2InventoryItemSetConsumeAction;
            ...
            if (context == null) {
                ...
                if (context.transform.GetComponentInParent<Gs2InventoryOwnItemSetList>(true) != null) {
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.ObjectField("Context", context.gameObject, typeof(Gs2InventoryOwnItemSetContext), false);
                    EditorGUI.EndDisabledGroup();
                    EditorGUILayout.HelpBox("ItemSet is auto assign from Gs2InventoryItemSetList.", MessageType.Info);
                }
                else {
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.ObjectField("Context", context.gameObject, typeof(Gs2InventoryOwnItemSetContext), false);
                    EditorGUI.indentLevel++;
                    context.ItemSet = EditorGUILayout.ObjectField("OwnItemSet", context.ItemSet, typeof(OwnItemSet), false) as OwnItemSet;
