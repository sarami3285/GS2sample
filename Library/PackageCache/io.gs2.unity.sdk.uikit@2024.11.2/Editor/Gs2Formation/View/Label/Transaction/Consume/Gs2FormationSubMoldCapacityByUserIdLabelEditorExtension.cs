...
{
    [CustomEditor(typeof(Gs2FormationSubMoldCapacityByUserIdLabel))]
    public class Gs2FormationSubMoldCapacityByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2FormationSubMoldCapacityByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2FormationSubMoldCapacityByUserIdFetcher>() ?? original.GetComponentInParent<Gs2FormationSubMoldCapacityByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2FormationSubMoldCapacityByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2FormationSubMoldCapacityByUserIdFetcher>();
                }
            }
