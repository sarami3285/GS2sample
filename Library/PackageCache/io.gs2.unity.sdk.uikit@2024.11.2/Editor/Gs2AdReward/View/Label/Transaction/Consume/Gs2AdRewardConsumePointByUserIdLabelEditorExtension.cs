...
{
    [CustomEditor(typeof(Gs2AdRewardConsumePointByUserIdLabel))]
    public class Gs2AdRewardConsumePointByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2AdRewardConsumePointByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2AdRewardConsumePointByUserIdFetcher>() ?? original.GetComponentInParent<Gs2AdRewardConsumePointByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2AdRewardConsumePointByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2AdRewardConsumePointByUserIdFetcher>();
                }
            }
