...
namespace Gs2.Unity.UiKit.Gs2Stamina.Label.Editor
{
    ...
    public class Gs2StaminaConsumeStaminaByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2StaminaConsumeStaminaByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2StaminaConsumeStaminaByUserIdFetcher>() ?? original.GetComponentInParent<Gs2StaminaConsumeStaminaByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2StaminaConsumeStaminaByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2StaminaConsumeStaminaByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("ConsumeValue")) {
                original.format += "{consumeValue}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
