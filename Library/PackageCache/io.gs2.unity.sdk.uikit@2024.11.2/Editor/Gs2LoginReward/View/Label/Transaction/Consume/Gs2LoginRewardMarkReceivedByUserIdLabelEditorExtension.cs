...
{
    [CustomEditor(typeof(Gs2LoginRewardMarkReceivedByUserIdLabel))]
    public class Gs2LoginRewardMarkReceivedByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2LoginRewardMarkReceivedByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2LoginRewardMarkReceivedByUserIdFetcher>() ?? original.GetComponentInParent<Gs2LoginRewardMarkReceivedByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2LoginRewardMarkReceivedByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2LoginRewardMarkReceivedByUserIdFetcher>();
                }
            }
