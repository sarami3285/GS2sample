...
{
    [CustomEditor(typeof(Gs2MissionMissionGroupModelListFetcher))]
    public class Gs2MissionMissionGroupModelListFetcherEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2MissionMissionGroupModelListFetcher;

            if (original == null) return;

            var context = original.GetComponent<Gs2MissionNamespaceContext>() ?? original.GetComponentInParent<Gs2MissionNamespaceContext>(true);
            if (context == null) {
