...
{
    [CustomEditor(typeof(Gs2MissionDecreaseCounterByUserIdLabel))]
    public class Gs2MissionDecreaseCounterByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2MissionDecreaseCounterByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2MissionDecreaseCounterByUserIdFetcher>() ?? original.GetComponentInParent<Gs2MissionDecreaseCounterByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2MissionDecreaseCounterByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2MissionDecreaseCounterByUserIdFetcher>();
                }
            }
            ...
            if (userDataFetcher != null) {
                ...
                if (GUILayout.Button("UserData:Values:Changed")) {
                    original.format += "{userData:values:changed}";
                    GUI.FocusControl("");
                    EditorUtility.SetDirty(original);
                }
            }
