...
{
    [CustomEditor(typeof(Gs2MissionBatchReceiveByUserIdLabel))]
    public class Gs2MissionBatchReceiveByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2MissionBatchReceiveByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2MissionBatchReceiveByUserIdFetcher>() ?? original.GetComponentInParent<Gs2MissionBatchReceiveByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2MissionBatchReceiveByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2MissionBatchReceiveByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("MissionTaskNames")) {
                original.format += "{missionTaskNames}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
