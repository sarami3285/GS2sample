...
{
    [CustomEditor(typeof(Gs2MissionVerifyCompleteByUserIdLabel))]
    public class Gs2MissionVerifyCompleteByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2MissionVerifyCompleteByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2MissionVerifyCompleteByUserIdFetcher>() ?? original.GetComponentInParent<Gs2MissionVerifyCompleteByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2MissionVerifyCompleteByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2MissionVerifyCompleteByUserIdFetcher>();
                }
            }
