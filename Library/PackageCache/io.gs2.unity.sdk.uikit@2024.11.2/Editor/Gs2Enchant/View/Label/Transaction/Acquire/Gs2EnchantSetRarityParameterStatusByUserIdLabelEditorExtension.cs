...
{
    [CustomEditor(typeof(Gs2EnchantSetRarityParameterStatusByUserIdLabel))]
    public class Gs2EnchantSetRarityParameterStatusByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2EnchantSetRarityParameterStatusByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2EnchantSetRarityParameterStatusByUserIdFetcher>() ?? original.GetComponentInParent<Gs2EnchantSetRarityParameterStatusByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2EnchantSetRarityParameterStatusByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2EnchantSetRarityParameterStatusByUserIdFetcher>();
                }
            }
