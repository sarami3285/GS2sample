...
{
    [CustomEditor(typeof(Gs2EnchantReDrawRarityParameterStatusByUserIdLabel))]
    public class Gs2EnchantReDrawRarityParameterStatusByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2EnchantReDrawRarityParameterStatusByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2EnchantReDrawRarityParameterStatusByUserIdFetcher>() ?? original.GetComponentInParent<Gs2EnchantReDrawRarityParameterStatusByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2EnchantReDrawRarityParameterStatusByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2EnchantReDrawRarityParameterStatusByUserIdFetcher>();
                }
            }
