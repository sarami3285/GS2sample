...
{
    [CustomEditor(typeof(Gs2EnchantSetBalanceParameterStatusByUserIdLabel))]
    public class Gs2EnchantSetBalanceParameterStatusByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2EnchantSetBalanceParameterStatusByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2EnchantSetBalanceParameterStatusByUserIdFetcher>() ?? original.GetComponentInParent<Gs2EnchantSetBalanceParameterStatusByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2EnchantSetBalanceParameterStatusByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2EnchantSetBalanceParameterStatusByUserIdFetcher>();
                }
            }
