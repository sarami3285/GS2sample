...
{
    [CustomEditor(typeof(Gs2EnchantRarityParameterStatusVerifyRarityParameterStatusAction))]
    public class Gs2EnchantRarityParameterStatusVerifyRarityParameterStatusActionEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2EnchantRarityParameterStatusVerifyRarityParameterStatusAction;
            ...
            if (context == null) {
                ...
                if (context.transform.GetComponentInParent<Gs2EnchantOwnRarityParameterStatusList>(true) != null) {
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.ObjectField("Context", context.gameObject, typeof(Gs2EnchantOwnRarityParameterStatusContext), false);
                    EditorGUI.EndDisabledGroup();
                    EditorGUILayout.HelpBox("RarityParameterStatus is auto assign from Gs2EnchantRarityParameterStatusList.", MessageType.Info);
                }
                else {
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.ObjectField("Context", context.gameObject, typeof(Gs2EnchantOwnRarityParameterStatusContext), false);
                    EditorGUI.indentLevel++;
                    context.RarityParameterStatus = EditorGUILayout.ObjectField("OwnRarityParameterStatus", context.RarityParameterStatus, typeof(OwnRarityParameterStatus), false) as OwnRarityParameterStatus;
