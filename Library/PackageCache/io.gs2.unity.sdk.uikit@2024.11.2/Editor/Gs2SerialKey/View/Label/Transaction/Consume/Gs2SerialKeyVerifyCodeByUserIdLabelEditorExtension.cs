...
{
    [CustomEditor(typeof(Gs2SerialKeyVerifyCodeByUserIdLabel))]
    public class Gs2SerialKeyVerifyCodeByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2SerialKeyVerifyCodeByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2SerialKeyVerifyCodeByUserIdFetcher>() ?? original.GetComponentInParent<Gs2SerialKeyVerifyCodeByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2SerialKeyVerifyCodeByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2SerialKeyVerifyCodeByUserIdFetcher>();
                }
            }
