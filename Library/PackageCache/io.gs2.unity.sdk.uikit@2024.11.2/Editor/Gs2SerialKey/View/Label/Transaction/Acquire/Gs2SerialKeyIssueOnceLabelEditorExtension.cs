...
{
    [CustomEditor(typeof(Gs2SerialKeyIssueOnceLabel))]
    public class Gs2SerialKeyIssueOnceLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2SerialKeyIssueOnceLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2SerialKeyIssueOnceFetcher>() ?? original.GetComponentInParent<Gs2SerialKeyIssueOnceFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2SerialKeyIssueOnceFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2SerialKeyIssueOnceFetcher>();
                }
            }
            if (userDataFetcher == null) {
                EditorGUILayout.HelpBox("Gs2SerialKeyOwnSerialKeyFetcher not found. Adding a Fetcher allows more values to be used.", MessageType.Warning);
                if (GUILayout.Button("Add Fetcher")) {
