...
{
    [CustomEditor(typeof(Gs2ExperienceMultiplyAcquireActionsByUserIdLabel))]
    public class Gs2ExperienceMultiplyAcquireActionsByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ExperienceMultiplyAcquireActionsByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ExperienceMultiplyAcquireActionsByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ExperienceMultiplyAcquireActionsByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ExperienceMultiplyAcquireActionsByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ExperienceMultiplyAcquireActionsByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("BaseRate")) {
                original.format += "{baseRate}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
