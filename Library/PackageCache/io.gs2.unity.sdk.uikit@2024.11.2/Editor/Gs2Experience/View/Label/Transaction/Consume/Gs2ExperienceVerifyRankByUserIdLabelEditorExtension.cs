...
{
    [CustomEditor(typeof(Gs2ExperienceVerifyRankByUserIdLabel))]
    public class Gs2ExperienceVerifyRankByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ExperienceVerifyRankByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ExperienceVerifyRankByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ExperienceVerifyRankByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ExperienceVerifyRankByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ExperienceVerifyRankByUserIdFetcher>();
                }
            }
