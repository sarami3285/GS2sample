...
{
    [CustomEditor(typeof(Gs2ExperienceSetExperienceByUserIdLabel))]
    public class Gs2ExperienceSetExperienceByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ExperienceSetExperienceByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ExperienceSetExperienceByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ExperienceSetExperienceByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ExperienceSetExperienceByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ExperienceSetExperienceByUserIdFetcher>();
                }
            }
            ...
            if (userDataFetcher != null) {
                ...
                if (GUILayout.Button("UserData:ExperienceValue:Changed")) {
                    original.format += "{userData:experienceValue:changed}";
                    GUI.FocusControl("");
                    EditorUtility.SetDirty(original);
                }
