...
{
    [CustomEditor(typeof(Gs2ExperienceVerifyRankCapByUserIdLabel))]
    public class Gs2ExperienceVerifyRankCapByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ExperienceVerifyRankCapByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ExperienceVerifyRankCapByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ExperienceVerifyRankCapByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ExperienceVerifyRankCapByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ExperienceVerifyRankCapByUserIdFetcher>();
                }
            }
