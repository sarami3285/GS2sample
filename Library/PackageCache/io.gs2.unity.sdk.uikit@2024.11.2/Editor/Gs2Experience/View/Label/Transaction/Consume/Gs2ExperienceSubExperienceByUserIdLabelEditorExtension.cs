...
{
    [CustomEditor(typeof(Gs2ExperienceSubExperienceByUserIdLabel))]
    public class Gs2ExperienceSubExperienceByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ExperienceSubExperienceByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ExperienceSubExperienceByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ExperienceSubExperienceByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ExperienceSubExperienceByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ExperienceSubExperienceByUserIdFetcher>();
                }
            }
