...
namespace Gs2.Unity.UiKit.Gs2StateMachine.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/StateMachine/Status/View/Enabler/Properties/Status/Gs2StateMachineOwnStatusStatusEnabler")]
    public partial class Gs2StateMachineOwnStatusStatusEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableStatuses.Contains(this._fetcher.Status?.Status ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableStatuses.Contains(this._fetcher.Status?.Status ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.Status?.Status ?? "").StartsWith(this.enableStatus));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.Status?.Status ?? "").EndsWith(this.enableStatus));
                    break;
                default:
    ...
    public partial class Gs2StateMachineOwnStatusStatusEnabler
    {
