...
namespace Gs2.Unity.UiKit.Gs2StateMachine.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/StateMachine/Status/View/Enabler/Properties/Name/Gs2StateMachineOwnStatusNameEnabler")]
    public partial class Gs2StateMachineOwnStatusNameEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableNames.Contains(this._fetcher.Status?.Name ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableNames.Contains(this._fetcher.Status?.Name ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.Status?.Name ?? "").StartsWith(this.enableName));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.Status?.Name ?? "").EndsWith(this.enableName));
                    break;
                default:
    ...
    public partial class Gs2StateMachineOwnStatusNameEnabler
    {
