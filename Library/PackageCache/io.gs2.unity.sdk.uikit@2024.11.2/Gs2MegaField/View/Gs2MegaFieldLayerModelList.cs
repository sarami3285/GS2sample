...
namespace Gs2.Unity.UiKit.Gs2MegaField
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.LayerModels.Count) {
                    this._children[i].SetLayerModel(
                        LayerModel.New(
    ...
    public partial class Gs2MegaFieldLayerModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2MegaFieldLayerModelListFetcher>() ?? GetComponentInParent<Gs2MegaFieldLayerModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2MegaFieldLayerModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2MegaFieldLayerModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2MegaFieldLayerModelListFetcher>() ?? GetComponentInParent<Gs2MegaFieldLayerModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
