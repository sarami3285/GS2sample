...
namespace Gs2.Unity.UiKit.Gs2Formation
{
	...
	[AddComponentMenu("GS2 UIKit/Formation/Form/View/Label/Gs2FormationOwnFormLabel")]
    public partial class Gs2FormationOwnFormLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{name}", $"{this._fetcher?.Form?.Name}"
                ).Replace(
                    "{index}", $"{this._fetcher?.Form?.Index}"
                ).Replace(
                    "{slots}", $"{this._fetcher?.Form?.Slots}"
                )
            );
        }
    }
    ...
    public partial class Gs2FormationOwnFormLabel
    {
