...
namespace Gs2.Unity.UiKit.Gs2Formation
{
    ...
    public partial class Gs2FormationOwnMoldList
    {
        ...
        {
            if (this.prefab == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2FormationOwnMoldContext Prefab.");
                enabled = false;
                return;
            }

            this._fetcher = GetComponent<Gs2FormationOwnMoldListFetcher>() ?? GetComponentInParent<Gs2FormationOwnMoldListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2FormationOwnMoldListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2FormationOwnMoldContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2FormationOwnMoldListFetcher>() ?? GetComponentInParent<Gs2FormationOwnMoldListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
