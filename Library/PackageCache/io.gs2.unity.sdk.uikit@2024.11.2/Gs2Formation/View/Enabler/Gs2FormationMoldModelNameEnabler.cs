...
namespace Gs2.Unity.UiKit.Gs2Formation.Enabler
{
    ...
    public partial class Gs2FormationMoldModelNameEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableNames.Contains(this._fetcher.MoldModel?.Name ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableNames.Contains(this._fetcher.MoldModel?.Name ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.MoldModel?.Name ?? "").StartsWith(this.enableName));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.MoldModel?.Name ?? "").EndsWith(this.enableName));
                    break;
                default:
