...
namespace Gs2.Unity.UiKit.Gs2Formation.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Formation/MoldModel/View/Enabler/Properties/Metadata/Gs2FormationMoldModelMetadataEnabler")]
    public partial class Gs2FormationMoldModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.MoldModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.MoldModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.MoldModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.MoldModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
