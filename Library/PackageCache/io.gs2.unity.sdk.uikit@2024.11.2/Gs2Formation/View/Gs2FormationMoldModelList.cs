...
namespace Gs2.Unity.UiKit.Gs2Formation
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.MoldModels.Count) {
                    this._children[i].SetMoldModel(
                        MoldModel.New(
                            this._fetcher.Context.Namespace,
                            this._fetcher.MoldModels[i].Name
                        )
                    );
    ...
    public partial class Gs2FormationMoldModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2FormationMoldModelListFetcher>() ?? GetComponentInParent<Gs2FormationMoldModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2FormationMoldModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2FormationMoldModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2FormationMoldModelListFetcher>() ?? GetComponentInParent<Gs2FormationMoldModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
