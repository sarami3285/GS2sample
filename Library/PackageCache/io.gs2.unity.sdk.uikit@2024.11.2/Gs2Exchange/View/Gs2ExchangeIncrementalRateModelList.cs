...
namespace Gs2.Unity.UiKit.Gs2Exchange
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.IncrementalRateModels.Count) {
                    this._children[i].SetIncrementalRateModel(
    ...
    public partial class Gs2ExchangeIncrementalRateModelList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2ExchangeIncrementalRateModelListFetcher>() ?? GetComponentInParent<Gs2ExchangeIncrementalRateModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2ExchangeIncrementalRateModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2ExchangeIncrementalRateModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2ExchangeIncrementalRateModelListFetcher>() ?? GetComponentInParent<Gs2ExchangeIncrementalRateModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
