...
namespace Gs2.Unity.UiKit.Gs2Lottery.SpriteSwitcher
{
    ...
    public partial class Gs2LotteryLotteryModelNameSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyNames.Contains(this._fetcher.LotteryModel?.Name ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.LotteryModel?.Name ?? "").StartsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.LotteryModel?.Name ?? "").EndsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
