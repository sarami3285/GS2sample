...
namespace Gs2.Unity.UiKit.Gs2Inventory.SpriteSwitcher
{
    ...
    public partial class Gs2InventoryInventoryModelNameSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyNames.Contains(this._fetcher.InventoryModel?.Name ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.InventoryModel?.Name ?? "").StartsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.InventoryModel?.Name ?? "").EndsWith(this.applyName)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
