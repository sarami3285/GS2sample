...
namespace Gs2.Unity.UiKit.Gs2Inventory.SpriteSwitcher
{
    ...
    public partial class Gs2InventorySimpleItemModelMetadataSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyMetadatas.Contains(this._fetcher.SimpleItemModel?.Metadata ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.SimpleItemModel?.Metadata ?? "").StartsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.SimpleItemModel?.Metadata ?? "").EndsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
