...
namespace Gs2.Unity.UiKit.Gs2Inventory
{
	...
	[AddComponentMenu("GS2 UIKit/Inventory/ItemModel/View/Label/Gs2InventoryItemModelLabel")]
    public partial class Gs2InventoryItemModelLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{name}", $"{this._fetcher?.ItemModel?.Name}"
                ).Replace(
                    "{metadata}", $"{this._fetcher?.ItemModel?.Metadata}"
                ).Replace(
                    "{stackingLimit}", $"{this._fetcher?.ItemModel?.StackingLimit}"
                ).Replace(
                    "{allowMultipleStacks}", $"{this._fetcher?.ItemModel?.AllowMultipleStacks}"
                ).Replace(
                    "{sortValue}", $"{this._fetcher?.ItemModel?.SortValue}"
                )
            );
        }
    }
