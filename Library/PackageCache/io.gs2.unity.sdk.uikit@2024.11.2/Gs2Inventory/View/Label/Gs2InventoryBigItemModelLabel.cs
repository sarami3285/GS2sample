...
namespace Gs2.Unity.UiKit.Gs2Inventory
{
	...
	[AddComponentMenu("GS2 UIKit/Inventory/BigItemModel/View/Label/Gs2InventoryBigItemModelLabel")]
    public partial class Gs2InventoryBigItemModelLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{name}", $"{this._fetcher?.BigItemModel?.Name}"
                ).Replace(
                    "{metadata}", $"{this._fetcher?.BigItemModel?.Metadata}"
                )
            );
        }
    }
    ...
    public partial class Gs2InventoryBigItemModelLabel
    {
