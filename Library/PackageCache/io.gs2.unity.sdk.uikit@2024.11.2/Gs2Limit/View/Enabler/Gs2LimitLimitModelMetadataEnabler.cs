...
namespace Gs2.Unity.UiKit.Gs2Limit.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Limit/LimitModel/View/Enabler/Properties/Metadata/Gs2LimitLimitModelMetadataEnabler")]
    public partial class Gs2LimitLimitModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.LimitModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.LimitModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.LimitModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.LimitModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
