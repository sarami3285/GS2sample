...
namespace Gs2.Unity.UiKit.Gs2LoginReward
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.BonusModels.Count) {
                    this._children[i].SetBonusModel(
                        BonusModel.New(
    ...
    public partial class Gs2LoginRewardBonusModelList
    {
        ...
        {
            if (this.prefab == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2LoginRewardBonusModelContext Prefab.");
                enabled = false;
                return;
            }

            this._fetcher = GetComponent<Gs2LoginRewardBonusModelListFetcher>() ?? GetComponentInParent<Gs2LoginRewardBonusModelListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2LoginRewardBonusModelListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2LoginRewardBonusModelContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2LoginRewardBonusModelListFetcher>() ?? GetComponentInParent<Gs2LoginRewardBonusModelListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
