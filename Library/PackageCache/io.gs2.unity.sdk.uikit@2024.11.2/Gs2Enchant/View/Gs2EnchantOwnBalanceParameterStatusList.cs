...
namespace Gs2.Unity.UiKit.Gs2Enchant
{
        ...
        private void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.BalanceParameterStatuses.Count) {
                    this._children[i].SetOwnBalanceParameterStatus(
    ...
    public partial class Gs2EnchantOwnBalanceParameterStatusList
    {
        ...
        public void Awake()
        {
            ...
            this._fetcher = GetComponent<Gs2EnchantOwnBalanceParameterStatusListFetcher>() ?? GetComponentInParent<Gs2EnchantOwnBalanceParameterStatusListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2EnchantOwnBalanceParameterStatusListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2EnchantOwnBalanceParameterStatusContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2EnchantOwnBalanceParameterStatusListFetcher>() ?? GetComponentInParent<Gs2EnchantOwnBalanceParameterStatusListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
