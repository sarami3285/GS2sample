...
namespace Gs2.Unity.UiKit.Gs2Enchant.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Enchant/RarityParameterModel/View/Enabler/Properties/Name/Gs2EnchantRarityParameterModelNameEnabler")]
    public partial class Gs2EnchantRarityParameterModelNameEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableNames.Contains(this._fetcher.RarityParameterModel?.Name ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableNames.Contains(this._fetcher.RarityParameterModel?.Name ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.RarityParameterModel?.Name ?? "").StartsWith(this.enableName));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.RarityParameterModel?.Name ?? "").EndsWith(this.enableName));
                    break;
                default:
    ...
    public partial class Gs2EnchantRarityParameterModelNameEnabler
    {
