...
namespace Gs2.Unity.UiKit.Gs2Enchant.Enabler
{
    ...
    public partial class Gs2EnchantRarityParameterModelMetadataEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableMetadatas.Contains(this._fetcher.RarityParameterModel?.Metadata ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableMetadatas.Contains(this._fetcher.RarityParameterModel?.Metadata ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.RarityParameterModel?.Metadata ?? "").StartsWith(this.enableMetadata));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.RarityParameterModel?.Metadata ?? "").EndsWith(this.enableMetadata));
                    break;
                default:
