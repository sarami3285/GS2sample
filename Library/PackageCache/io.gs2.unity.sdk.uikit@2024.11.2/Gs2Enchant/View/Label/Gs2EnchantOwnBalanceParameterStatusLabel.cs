...
namespace Gs2.Unity.UiKit.Gs2Enchant
{
	...
	[AddComponentMenu("GS2 UIKit/Enchant/BalanceParameterStatus/View/Label/Gs2EnchantOwnBalanceParameterStatusLabel")]
    public partial class Gs2EnchantOwnBalanceParameterStatusLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{parameterName}", $"{this._fetcher?.BalanceParameterStatus?.ParameterName}"
                ).Replace(
                    "{propertyId}", $"{this._fetcher?.BalanceParameterStatus?.PropertyId}"
                ).Replace(
                    "{parameterValues}", $"{this._fetcher?.BalanceParameterStatus?.ParameterValues}"
                )
            );
        }
    }
    ...
    public partial class Gs2EnchantOwnBalanceParameterStatusLabel
    {
