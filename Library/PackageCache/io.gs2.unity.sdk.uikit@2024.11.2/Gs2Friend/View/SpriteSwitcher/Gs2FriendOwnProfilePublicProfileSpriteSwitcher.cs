...
namespace Gs2.Unity.UiKit.Gs2Friend.SpriteSwitcher
{
    ...
    public partial class Gs2FriendOwnProfilePublicProfileSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyPublicProfiles.Contains(this._fetcher.Profile?.PublicProfile ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.Profile?.PublicProfile ?? "").StartsWith(this.applyPublicProfile)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.Profile?.PublicProfile ?? "").EndsWith(this.applyPublicProfile)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
