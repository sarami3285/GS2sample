...
namespace Gs2.Unity.UiKit.Gs2Friend.Enabler
{
    ...
    public partial class Gs2FriendOwnFriendUserPublicProfileEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enablePublicProfiles.Contains(this._fetcher.FriendUser?.PublicProfile ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enablePublicProfiles.Contains(this._fetcher.FriendUser?.PublicProfile ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.FriendUser?.PublicProfile ?? "").StartsWith(this.enablePublicProfile));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.FriendUser?.PublicProfile ?? "").EndsWith(this.enablePublicProfile));
                    break;
                default:
