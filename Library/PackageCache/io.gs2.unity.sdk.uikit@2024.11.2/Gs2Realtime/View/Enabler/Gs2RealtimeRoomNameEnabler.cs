...
namespace Gs2.Unity.UiKit.Gs2Realtime.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Realtime/Room/View/Enabler/Properties/Name/Gs2RealtimeRoomNameEnabler")]
    public partial class Gs2RealtimeRoomNameEnabler : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    this.target.SetActive(this.enableNames.Contains(this._fetcher.Room?.Name ?? ""));
                    break;
                case Expression.NotIn:
                    this.target.SetActive(!this.enableNames.Contains(this._fetcher.Room?.Name ?? ""));
                    break;
                case Expression.StartsWith:
                    this.target.SetActive((this._fetcher.Room?.Name ?? "").StartsWith(this.enableName));
                    break;
                case Expression.EndsWith:
                    this.target.SetActive((this._fetcher.Room?.Name ?? "").EndsWith(this.enableName));
                    break;
                default:
