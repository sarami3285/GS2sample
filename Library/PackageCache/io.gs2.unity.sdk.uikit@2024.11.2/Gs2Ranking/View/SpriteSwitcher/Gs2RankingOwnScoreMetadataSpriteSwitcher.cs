...
namespace Gs2.Unity.UiKit.Gs2Ranking.SpriteSwitcher
{
    ...
    public partial class Gs2RankingOwnScoreMetadataSpriteSwitcher : MonoBehaviour
    {
        private void OnFetched()
        {
            ...
            {
                case Expression.In:
                    if (this.applyMetadatas.Contains(this._fetcher.Score?.Metadata ?? "")) {
                ...
                case Expression.StartsWith:
                    if ((this._fetcher.Score?.Metadata ?? "").StartsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                case Expression.EndsWith:
                    if ((this._fetcher.Score?.Metadata ?? "").EndsWith(this.applyMetadata)) {
                        this.onUpdate.Invoke(this.sprite);
                    }
                    break;
                default:
